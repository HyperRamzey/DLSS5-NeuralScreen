...
def zip_integrity():
    zpath = ROOT / "neuralscreen-v1.8.1-full.zip"
    if not zpath.is_file():
        return False, "no neuralscreen-v1.8.1-full.zip"
    required = [
    ...
    with zipfile.ZipFile(zpath) as z:
        ...
        if "NeuralScreen 1.8.1" not in vt:
            return False, "VERSION.txt version does not match v1.8.1"
