...
FG-рантайм (`nvngx_dlssg.dll`) **не поставляется**: та же лицензионная
позиция, что и у SR - редистрибутив NVIDIA нельзя включать в архив. Положите
его в `native/libraries/` (README внутри); без DLL переключатель вежливо
откажет, ничего не ломается.
